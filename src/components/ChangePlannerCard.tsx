...
export function ChangePlannerCard({ plan, onGeneratePrompt, onGeneratePhasePrompt }: ChangePlannerCardProps) {
      ...
      {isBigChange ? (
        <div className="rounded-xl border border-orange-200 bg-orange-50 p-3">
          ...
          <p className="mt-1 text-xs text-orange-700">
            Do not send this as one giant task. Work through each phase in order. Generate a prompt per phase.
          </p>
        </div>
      ) : (
        <button
          ...
          className="min-h-12 w-full rounded-xl bg-brand px-4 text-sm font-semibold text-white"
        >
          Generate Prompt
        </button>
      )}
